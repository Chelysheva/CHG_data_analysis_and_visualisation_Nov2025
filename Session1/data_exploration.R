...
# OR load the csv file that is in your folder
...
View(owid_covid[owid_covid$location == "United Kingdom", ])
# SORT data ####
...
owid_covid %>%
  arrange(date, desc(location)) -> owid_covid_sorted_by_date_location
...
owid_covid$prop_new_deaths_to_cases = owid_covid$new_deaths/owid_covid$new_cases
...
owid_covid_hdi_class_new_year<-owid_covid_hdi_class[owid_covid_hdi_class$date=="2021-01-01",]
plot(x = owid_covid_hdi_class_new_year$total_cases_per_million, y = owid_covid_hdi_class_new_year$total_deaths_per_million, col = owid_covid_hdi_class_new_year$hdi_class)
