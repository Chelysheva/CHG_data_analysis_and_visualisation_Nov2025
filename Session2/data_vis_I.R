...
owid_covid = read.csv("Session2/data/owid-covid-data.csv", header = TRUE,
...
ggplot(owid_covid_uk) +
  geom_line(aes(y = total_cases_per_million, x = date), colour = "red") +
  geom_line(aes(x = date, y = total_deaths_per_million), colour = "blue") 
...
# 3 - add geom(s)
ggplot(owid_covid_newyear, aes(x = log(total_cases), y = log(total_deaths))) +
  geom_point()
...
ggplot(owid_covid_newyear, aes(x = total_cases, y = total_deaths)) +
  ...
  geom_smooth(method = "lm") +
  theme_bw() +
  ggtitle(label = "", subtitle = "OWID Covid dataset: Total cases vs. total deaths on Jan 01, 2021")
...
ggplot(owid_covid_newyear, aes(x = total_cases, y = total_deaths)) +
  geom_point(aes(size = people_vaccinated)) +
...
ggsave("plot_total_cases_vs_deaths.pdf", 
       plot_total_cases_vs_deaths, width = 20, height = 5)
